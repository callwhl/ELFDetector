...
public class ElfManager {
	...
	public ElfManager(MappedByteBuffer fileMemMap)throws Exception{
		...
		byte[] fileMemArray=new byte[fileMemMap.limit()];
		for(int i=0;i<fileMemMap.limit();i++){
			fileMemArray[i]=fileMemMap.get();
		}
		//读取ELF header
		...
		Elf32_Off pht_offset=elf_header.getE_phoff();
		for(int i=0;i<ConvertUtil.byteArrayToInt(elf_header.getE_phnum().getValue(),MachineCharacter.LITTLE_ENDIAN);i++){
			...
			program_header.setP_type(new Elf32_Word(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_offset(new Elf32_Off(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_vaddr(new Elf32_Addr(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_paddr(new Elf32_Addr(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_filesz(new Elf32_Word(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_memsz(new Elf32_Word(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_flags(new Elf32_Word(fileMemArray,pht_offset));
			pht_offset.add(4);
			program_header.setP_align(new Elf32_Word(fileMemArray,pht_offset));
	...
	public void printElfHeader(){
		...
		System.out.println("e_type:"+Recognizer.recognizeElf32Type(elf_header.getE_type().getHexString(MachineCharacter.LITTLE_ENDIAN)));
		System.out.println("e_machine:"+Recognizer.recognizeElf32Machine(elf_header.getE_machine().getHexString(MachineCharacter.LITTLE_ENDIAN)));
		System.out.println("e_version:"+elf_header.getE_version().getHexString(MachineCharacter.LITTLE_ENDIAN));
